...
int main(void)
{
	...
	printf("\tinitloop:\n");
	printf("\t\t%%countval = load i32* %%countRef\n");
	printf("\t\t%%countvalInc = add i32 %%countval, 1\n");
	printf("\t\t%%byteRef = getelementptr i8* %%mem, i32 %%countval\n");
	...
	do {
		...
		switch (c) {
			case '>':
				...
				printf("\t%%offset%d = load i32* %%offsetPtr\n", r);
			...
			case '+':
				...
				printf("\t%%offset%d = load i32* %%offsetPtr\n", r);
				printf("\t%%valuePtr%d = getelementptr i8* %%mem, i32 %%offset%d\n", r, r);
				printf("\t%%value%d = load i8* %%valuePtr%d\n", r, r);
